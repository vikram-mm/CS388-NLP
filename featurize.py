...
class Featurizer():
    ...
    def __init__(self, train_class_exs: List[PersonExample]):
        ...
        self.num_dimensions = 11 + self.vocab_size + 1
    
    def featurize(self, tokens: List[str], labels, frequency_threshold=40, oneBefore_threshold=18,\
     twoBefore_threshold = 16, oneAfter_threshold=2):
        ...
        for i, token in enumerate(tokens):
            ...
            feature = np.zeros(11 + vocab_size + 1)
            ...
            feature[6] = int(re.match('^[a-zA-Z][\w-]+$', token) is not None) * len(token)
            ...
            if token in self.vocab_dict:
                feature[11 + self.vocab_dict[token]] = 1
                gradient_keys.append(11 + self.vocab_dict[token])

            if i >= 1 and tokens[i-1] in self.vocab_dict:
                feature[11 + self.vocab_dict[tokens[i-1]]] = 1
                gradient_keys.append(11  + self.vocab_dict[tokens[i-1]])

            if i+1 < len(tokens) and tokens[i+1] in self.vocab_dict:
                feature[11 + self.vocab_dict[tokens[i+1]]] = 1
                gradient_keys.append(11 + self.vocab_dict[tokens[i+1]])

            feature[-1] = 1 #for bias
        ...
        return features_list, gradient_key_list, labels_list

    def featurize_oneInstance(self, tokens: List[str], idx, frequency_threshold=40, oneBefore_threshold=18,\
     twoBefore_threshold = 16, oneAfter_threshold=2):
