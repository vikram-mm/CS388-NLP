...
class Featurizer():
    ...
    def __init__(self, train_class_exs: List[PersonExample]):
        ...
        self.num_dimensions = 14 + 1 + 3*self.vocab_size + 1#plus one for bias, plus one for none of the words
        self.num_direct_dimensions = 14 + 1 # 1 is bias
     ...
     twoBefore_threshold = 16, oneAfter_threshold=2):
        ...
        for i, token in enumerate(tokens):
            ...
            if token.lower() in self.frequentNameDict:
                feature[6] = self.frequentNameDict[token.lower()]/ self.max_frequentNameDict
            
            if token.lower() in self.frequencyDict:
                feature[7] = self.frequencyDict[token.lower()]/ self.max_frequencyDict
 
            if i>=1 and tokens[i-1].lower() in self.oneBeforeNameDict:
                feature[8] = self.oneBeforeNameDict[tokens[i-1].lower()]/ self.max_oneBeforeNameDict

            if i>=2 and tokens[i-2].lower() in self.twoBeforeNameDict:
                feature[9] = self.twoBeforeNameDict[tokens[i-2].lower()]/ self.max_twoBeforeNameDict
            
            if i+1 < len(tokens) and tokens[i+1].lower() in self.oneAfterNameDict:
                feature[10] = self.oneAfterNameDict[tokens[i+1].lower()]/ self.max_oneAfterNameDict


            #tf - idf features
            feature[11] = self.get_tf_idf(token.lower())
            if i>= 1:
                feature[12] = self.get_tf_idf(tokens[i-1].lower())
            if i+1 < len(tokens):
            ...
            #bag of word indices
            feature[15:] = 15 + 3*self.vocab_size - 1#words not in vocab

            if token.lower() in self.vocab_dict:
                feature[15] = 12 + self.vocab_dict[token.lower()]

            if i >= 1 and tokens[i-1].lower() in self.vocab_dict:
                feature[16] = 12 + self.vocab_dict[tokens[i-1].lower()] + self.vocab_size

                
            if i+1 < len(tokens) and tokens[i+1].lower() in self.vocab_dict:
                feature[17] = 12 + self.vocab_dict[tokens[i+1].lower()] + 2*self.vocab_size
