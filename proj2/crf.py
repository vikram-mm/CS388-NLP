...
class CRF(nn.Module):
    ...
    def __init__(
        ...
        self.num_features = num_features
        # self.emmision_weights = []
        # for i in range(num_features):
        #     self.emmision_weights.append(nn.Parameter(torch.empty(self.nb_labels)))
        #     nn.init.uniform_(self.emmision_weights[i], -0.1, 0.1)
        self.init_weights()
    ...
    def get_emssions(self, seq_x):
        ...
        # emmisions = torch.zeros(1, seq_length, self.nb_labels)
        emmisions = []
        # for seq_idx in range(seq_length):
        #     for idx in range(num_active_indexes):
        #         feature_idx  = x[seq_idx, : , idx]
        #         print(feature_idx)
        #         exit(0)
        #         emmisions[0, seq_idx, :] += self.emmision_weights[feature_idx][label_idx]
        ...
        for i, x in enumerate(seq_x):
            ...
            # print(self.emmision_weights.shape)
            potential = torch.sparse.mm(features, self.emmision_weights)

            # emmisions[0, i, :] += potential.squeeze()
    ...
    def forward(self, x):
        ...
        score, path = self.decode(emissions)
        return path
